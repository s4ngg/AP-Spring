...
public class OrderServiceImpl implements OrderService {
    ...
    public DeliveryAddressResponseDto addDeliveryAddress(Long memberId, DeliveryAddressRequestDto request) {
        ...
        if (request.isDefault()) {
            deliveryAddressRepository.findByMemberIdAndDeletedAtIsNull(memberId)
                    .forEach(addr -> {
                        if (addr.isDefault()) {
                            addr.update(addr.getRecipientName(), addr.getPhone(), addr.getZipCode(),
                                    addr.getAddress(), addr.getAddressDetail(), false);
                        }
                    });
        }
    ...
    public DeliveryAddressResponseDto updateDeliveryAddress(Long memberId, Long addressId, DeliveryAddressRequestDto request) {
        ...
        if (request.isDefault()) {
            deliveryAddressRepository.findByMemberIdAndDeletedAtIsNull(memberId)
                    .forEach(addr -> {
                        if (addr.isDefault() && !addr.getAddressId().equals(addressId)) {
                            addr.update(addr.getRecipientName(), addr.getPhone(), addr.getZipCode(),
                                    addr.getAddress(), addr.getAddressDetail(), false);
                        }
                    });
