...
public class SellerApplyServiceImpl implements SellerApplyService {
    ...
    @Override
    public void apply(SellerApplyRequestDto dto, Long memberId) {  // ??apply ë©”ì„œ???œìž‘
    	if (sellerRepository.existsByMember_Id(memberId)) {
            logger.warn("[SellerApplyService] ?´ë? ?ë§¤??? ì²­???Œì› - memberId: {}", memberId);
    	...
    	if (sellerRepository.existsByBusinessNumber(dto.getBusinessNumber())) {
            logger.warn("[SellerApplyService] ?¬ì—…?ë“±ë¡ë²ˆ??ì¤‘ë³µ - businessNumber: {}", dto.getBusinessNumber());
        ...
        logger.info("[SellerApplyService] ?ë§¤??? ì²­ ?„ë£Œ - memberId: {}", memberId);
