...
public interface AttachmentControllerDocs {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "파일 없음 또는 이미지 파일 아님"),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 문의")
    })
    ResponseEntity<co.kr.allpick.global.response.ApiResponse<AttachmentResponseDto>> uploadInquiryAttachment(
    ...
    ResponseEntity<co.kr.allpick.global.response.ApiResponse<AttachmentResponseDto>> uploadClaimAttachment(
    ...
    ResponseEntity<co.kr.allpick.global.response.ApiResponse<List<AttachmentResponseDto>>> getByInquiryId(
            @Parameter(description = "문의 ID") @PathVariable("inquiryId") Long inquiryId);
    ...
    ResponseEntity<co.kr.allpick.global.response.ApiResponse<List<AttachmentResponseDto>>> getByClaimId(
            @Parameter(description = "클레임 ID") @PathVariable("claimId") Long claimId);
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "404", description = "존재하지 않는 첨부파일")
    })
    ResponseEntity<co.kr.allpick.global.response.ApiResponse<Void>> deleteAttachment(
