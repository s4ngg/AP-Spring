...
public interface AuthControllerDocs {
    ...
    @Operation(summary = "판매자 회원가입", description = "사업자 정보를 포함하여 판매자로 회원가입합니다.")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "판매자 회원가입 성공",
            content = @Content(examples = @ExampleObject(value = """
                {
                    "success": true,
                    "message": "판매자 회원가입 성공",
    ...
    ResponseEntity<ApiResponse<Void>> signupSeller(
        @RequestBody @Valid SignupSellerRequestDto dto);

    @Operation(summary = "일반 로그인", description = "이메일, 비밀번호로 로그인 후 JWT 토큰을 반환합니다.")
    @ApiResponses({
    ...
    @Operation(summary = "판매자 로그인", description = "판매자 전용 로그인입니다.")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "판매자 로그인 성공",
            content = @Content(examples = @ExampleObject(value = """
                {
                    "success": true,
                    "message": "판매자 로그인 성공",
    ...
    ResponseEntity<ApiResponse<AuthResponseDto>> loginSeller(
        @RequestBody @Valid LoginRequestDto dto);
}